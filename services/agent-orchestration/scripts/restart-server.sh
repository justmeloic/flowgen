...
# Also try to kill any process using port 8081
lsof -ti:8081 | xargs kill -9 2>/dev/null || true

# Wait and check if port is free
echo "Waiting for port 8081 to be freed..."
for i in {1..10}; do
...
# Final check
if lsof -i:8081 > /dev/null 2>&1; then
    echo "Error: Port 8081 is still in use after 10 seconds"
    exit 1
fi

echo "Starting server..."
# Start the server again using your exact command
nohup uvicorn src.app.main:app --reload --host 0.0.0.0 --port 8081 > nohup.out 2>&1 &
