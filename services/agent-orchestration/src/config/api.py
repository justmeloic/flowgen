...
# API configuration
flags.DEFINE_string('api_title', 'CBA Agent API', 'API title')
flags.DEFINE_string(
    'api_description',
    'API for interacting with the CBA information agent',
    'API description',
)
...
def get_settings() -> Dict[str, Any]:
    ...
    return {
        ...
        'cors': {
            ...
            'allow_methods': ['*'],
            'allow_headers': ['*', 'X-Session-ID'],  # Add explicit session header
            'expose_headers': ['X-Session-ID'],  # Expose session header for CORS
        },
        'model': {
